...
class ModelForInference:
    def __init__(
        ...
        if device.startswith("cuda"):
            ...
            ), "no cuda device detected, use device='cpu' instead"
            ...
            for idx, word in enumerate(example):
                ...
                if len(tokens) + len(tokenized_word) >= self.max_seq_len - 1:
                    ...
                    token_mappings.append(
                        [-1] + token_id_to_word_id + [-1]
                    )  # encounter for cls and sep tokens
    ...
    def merge_predictions(windows_preds, windows_scores, windows_token_mappings):
        # TODO: refactor
        result_size = (
            windows_token_mappings[-1][-2] + 1
        )  # element at -1 corresponds to </s> token, at -2 to last word idx
        results = [0] * result_size
        result_score = [0] * result_size

        prev_word_id = -1

        for window_preds, window_scores, window_token_mappings in zip(
...
if __name__ == "__main__":
    ...
    with open(args.path_to_test) as f, open(args.path_to_out, "w") as out:
        ...
        for idx, text in splitted:
            preds = ids_to_labels(
                model.predict([text.split()], context=args.context)[0], pred_labels
            )
